...
public class TestBase : IClassFixture<WebApplicationFactory<Program>>
{
  ...
  internal async Task<(HttpResponseMessage response, AuthResponseModel authModel)> RegistrationAsync(
    string login,
    string password
  )
  {
    ...
    var result = JsonSerializer.Deserialize<AuthResponseModel>(
  ...
  internal async Task<(HttpResponseMessage response, AuthResponseModel authModel)> LoginAsync(
    string login,
    string password
  )
  {
    ...
    var authModel = JsonSerializer.Deserialize<AuthResponseModel>(
      await response.Content.ReadAsStringAsync(),
      _jsonSerializerSettings
    );
  ...
  )
  {
    ...
    _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authResponseModel.AccessToken.Value);
    var result = JsonSerializer.Deserialize<AuthResponseModel>(
      await response.Content.ReadAsStringAsync(),
      _jsonSerializerSettings
    );
