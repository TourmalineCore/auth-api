...
public class UsersService
{
  ...
  public async Task RegisterAsync(RegistrationModel registrationModel)
  {
    ...
    {
      _logger.LogError($"[{nameof(UsersService)}]: Couldn't send a link on password creation for user [{registrationModel.CorporateEmail}]. Exception details: {ex.Message}");
    }
  }
  ...
  public async Task ChangePasswordAsync(PasswordChangeModel passwordChangeModel)
  {
    ...
    var passwordResetTokenIsValid = await _userManager.VerifyUserTokenAsync(
      user,
      _userManager.Options.Tokens.PasswordResetTokenProvider,
      UserManager<User>.ResetPasswordTokenPurpose,
      passwordChangeModel.PasswordResetToken
    );
