...
public class UserCredentialsValidator : IUserCredentialsValidator
{
  ...
  public async Task<bool> ValidateUserCredentials(string username, string password)
  {
    ...
    if (!isValidPassword)
    {
      _logger.LogWarning($"[{nameof(UserCredentialsValidator)}]: The password [{password}] is invalid for a user [{username}]");
      return false;
    }
    ...
    _logger.LogWarning($"[{nameof(UserCredentialsValidator)}]: User with credentials [{username}] was blocked.");
